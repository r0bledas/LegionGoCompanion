...
namespace HandheldCompanion.Managers
{
    public static class UpdateManager
    {
        ...
        static UpdateManager()
        {
            ...
            InstallPath = Path.Combine(System.IO.Path.Combine(System.Environment.GetFolderPath(System.Environment.SpecialFolder.ApplicationData), "HandheldCompanion"), "cache");
        ...
        private static async Task CheckGameControllerDb(bool background)
        {
            try
            {
                ...
                if (File.Exists("gamecontrollerdb.txt") && ComputeCrc32(cachedPath) == ComputeCrc32("gamecontrollerdb.txt"))
                {
        ...
        private static void ParseLatest(string contentsJson)
        {
            try
            {
                ...
                if (latestBuild <= new Version("1.0.0.0"))
                {
        ...
        public static bool InstallUpdate(UpdateFile updateFile)
        {
            if (updateFile.isGameControllerDb)
            {
                ...
                try
                {
                    File.Copy(cached, "gamecontrollerdb.txt", overwrite: true);
