...
public class ProfileManager : IManager
{
    ...
    {
        // initialize path
        ManagerPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "HandheldCompanion", "profiles");
    ...
    private void ProcessProfile(string fileName, bool imported = false)
    {
        ...
        if (imported)
        {
            ...
            UIHelper.TryInvoke(async () =>
            {
                ...
                ContentDialogResult result = ContentDialogResult.Primary; //await dialogTask; // await the task
